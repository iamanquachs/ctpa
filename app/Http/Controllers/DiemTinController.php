...
class DiemTinController extends Controller
{
    ...
    public function show_ChuyenMucDT($ID_CHUYENMUC_DT)
    {
        ...
        return view('page.DiemTin.CM_DiemTin')->with('chuyenmuc_ten', $chuyenmuc_ten)
            ->with('chuyenmucdiemtin', $chuyenmucdiemtin)->with('danhmuc', $danhmuc)->with('lienket', $lienket)->with('chuyenmucvanban', $chuyenmucvanban);
    }
    ...
    public function show_ChiTietDT($ID_DT)
    {
        ...
        if (count($diemtin) == 0) {
            ...
            return view('page.DiemTin.chitiet_Diemtin')->with('diemtin', $diemtin)
                ->with('chuyenmuc_ten', $chuyenmuc_ten)->with('lienket', $lienket)->with('chuyenmucvanban', $chuyenmucvb)
                ->with('DT_lienquan', $DT_lienquan);
        }
    }
    public function show_ChiTietTB($ID_DT)
    {
        $lienket = DB::table('lienketwebsite')->get();
        $chuyenmucvb = DB::table('chuyenmucvanban')->orderby('ID_CHUYENMUC_VB', 'desc')->get();
